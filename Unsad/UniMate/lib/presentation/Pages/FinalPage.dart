...
class _FinalPageState extends State<FinalPage> {
  ...
  Future<void> _loadLocalData() async {
    try {
      ...
      List<String>? storedList = prefs.getStringList('final_${userSemester}');
      if (storedList != null) {
  ...
  Future<void> _saveToLocal(List<Map<String, dynamic>> finalData) async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      List<String> strList = finalData.map((e) {
      ...
      await prefs.setStringList('final_${userSemester}', strList);
      localData = finalData;
    } catch (e) {
  ...
  Stream<List<Map<String, dynamic>>> _loadFinalStream() {
  ...
  Widget _buildFinalSection(List<Map<String, dynamic>> finalData, bool isAdmin) {
    _syncControllersWithData(finalData);

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Final Exams', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 4),
        ListView.builder(
          shrinkWrap: true,
          physics: const NeverScrollableScrollPhysics(),
          itemCount: finalData.length,
          itemBuilder: (_, index) => _buildSubjectRow(finalData[index], isAdmin),
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Final Page', style: TextStyle(fontWeight: FontWeight.bold)),
        backgroundColor: Colors.purpleAccent.shade700,
      ),
      body: StreamBuilder<List<Map<String, dynamic>>>(
        stream: _loadFinalStream(),
        initialData: localData,
        builder: (context, snapshot) {
          final data = snapshot.data ?? localData;
          return SingleChildScrollView(
            padding: const EdgeInsets.all(16),
            child: _buildFinalSection(data, isAdmin),
          );
        },
      ),
    );
